          ...
          <section
            ...
            <div class="border- mr-4 w-full text-center sm:w-1/2 sm:text-left">
              <ul class="mb-8 flex flex-col items-center space-y-1 dark:text-slate-400 sm:items-start">
                ...
                <form class="{{ $liClass }}" action="{{ route('personal-uai.update', $personal->id) }}" method="POST">
                  @method('PUT')
                  @csrf
                  ...
                  >
                    @foreach ($uai['cargo'] as $cargo)
                      <option value="{{ $cargo->id }}">{{ $cargo->nombre }}</option>
                    @endforeach
                  </select>
                  <li class="{{ $liClass }}">
                    <label for="recipient-p00" class="col-form-label">P00:</label>
                    <input id="recipient-p00" name="p00" type="text" class="{{ $inputClass }}"
                      value="{{ $personal->p00 }}" />
                  </li>
                  <li class="{{ $liClass }}">
                  ...
                  <li class="{{ $liClass }}">
                    <label for="recipient-gmail" class="col-form-label">Correo electrónico:</label>
                    <input 
                      id="recipient-gmail" 
                      name="gmail"
                      type="text" 
                  ...
                  <li class="{{ $liClass }}">
                    <label for="recipient-email_cantv" class="col-form-label">Correo Institucional:</label>
                    <input 
                      id="recipient-email_cantv" 
                      name="email_cantv"
                      type="text" 
                      class="{{ $inputClass }}"
                      value="{{ $personal->email_cantv }}" 
                    />
                  </li>
                  ...
                  <select 
                    ...
                    class="{{ $inputClass }}"
                    class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-blue-500 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 dark:focus:border-blue-500 dark:focus:ring-blue-500">
                    @foreach ($uai['departaments'] as $departament)
