<div role="acreditation">
    ...
    @isset($acreditation) 
        <x-secondary-button type="button" wire:click='getAcreditationDocument'>descargar acreditacion</x-secondary-button> 
    @endisset
    
    @can('auditActivity.show.designationAcreditation')
        {{-- todo Acredit --}}
        @empty($acreditation) 
            <x-secondary-button type='button' class="ml-4" x-on:click="$wire.openModalAcreditation = true"> {{ \__('Acreditar') }} </x-secondary-button> 
        @endempty
    @endcan
