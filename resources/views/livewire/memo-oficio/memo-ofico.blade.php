    ...
    <x-section-basic>
        ...
        <form id="hallazgosForm" method="POST" action="{{ route('guardar.memo') }}" onsubmit="return validateDates()">
            ...
            <select id="inputSelect" name="tipo_hallazgo" onchange="showDynamicInputs()">
                ...
                <option value="remision" {{ old('tipo_hallazgo') == 'remision' ? 'selected' : '' }}>Remisión del Informe Definitivo</option>
            </select>

            <div id="dynamicInputs" class="dynamic-inputs"></div>
        </form>

        <!-- Integrar los componentes Livewire para búsqueda -->
        <div class="button-container">
            <livewire:search-memo />
            <livewire:remision-definitivo />
        </div>
            ...
            function showDynamicInputs() {
                ...
                if (selectedValue === "ISEG") {
                    dynamicInputs.innerHTML = `
                        ...
                        <textarea name="hallazgos" rows="3" class="resize-vertical" placeholder="Ingrese los hallazgos aquí"></textarea>
                        <button type="submit" onclick="showSuccessMessage()">Guardar</button>
                        <button type="button" onclick="document.getElementById('hallazgosForm').reset(); document.getElementById('dynamicInputs').style.display = 'none';" class="bg-gray-600 text-white rounded-lg px-4 py-2 ml-2">Cerrar</button>
                    `;
                } else if (selectedValue === "remision") {
                    dynamicInputs.innerHTML = `
                        <label>Remisión del Informe Definitivo</label>
                        <div class="inline-inputs">
                            <input type="text" value="${new Date().getFullYear()}" disabled style="width: auto;" />
                            <span>-</span>
                            <input type="text" name="codigo" pattern="\\d{3}" required maxlength="3" placeholder="Número" oninput="fetchObjective(this.value)" />
                        </div>
                        <label>Para:</label>
                        <input type="text" name="para" required placeholder="Ingrese el destinatario" />
                        <label>Hallazgos:</label>
                        <textarea name="hallazgos" rows="3" class="resize-vertical" placeholder="Ingrese los hallazgos aquí"></textarea>
                        <label>Objective:</label>
                        <textarea name="objective" rows="3" class="resize-vertical" placeholder="Ingrese el objective aquí" readonly></textarea>
                        <button type="submit" formaction="{{ route('remision-definitivo.store') }}" onclick="showSuccessMessage()">Guardar</button>
                        <button type="button" onclick="document.getElementById('hallazgosForm').reset(); document.getElementById('dynamicInputs').style.display = 'none';" class="bg-gray-600 text-white rounded-lg px-4 py-2 ml-2">Cerrar</button>
                    `;
                    ...
                    dynamicInputs.style.display = "none"; // Ocultar la sección de hallazgos si no se selecciona nada
                }
            }
            ...
            function updateTable() {
                ...
                for (let i = 0; i < numRows; i++) {
                    ...
                    row.innerHTML = `
                        ...
                        <td><input type="text" name="fechas_reporte[]" required placeholder="Transferido a o Fecha" style="width: 100%;" onblur="validateDateOrText(this)"></td>
                    `;
                    tableBody.appendChild(row);
                }
            }
